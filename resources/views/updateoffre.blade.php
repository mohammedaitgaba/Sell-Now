@extends('layouts.app')
@section('update_offre')
    
    <section class="update_form">
        <div class="update_container">
            <form class="popup_update" enctype="multipart/form-data" method="POST" action="/updateoffre/{{$get_offre->id}} ">
                @csrf
                @method('PUT')
                <div class="popup_info">
                    <label for="">Tiltle</label>
                    <input type="text" name="title" placeholder="offre title" value=" {{$get_offre->title}}"
                ...
                <div class="popup_info">
                    ...
                    class="@error('description') border-red-500 @enderror" >{{$get_offre->description}}</textarea>
                    @error('description')
                         <div class="text-red-500 mt-2 text-sm"> {{$message}} </div>
                    @enderror
                </div>
